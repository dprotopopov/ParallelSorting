...
namespace ParallelSorting.Editor
{
    public partial class ChildForm : Form
    {
        ...
        public void Execute(int numberOfProcess, int gridSize, int blockSize, int degreeOfParallelism,
            ...
            ExecutionMethod executionMethod)
        {
            ...
            var commandFormat = (from object[] item in new object[]
                {
                    new object[]
                    {
                        ...
                        "/C mpiexec -n {0} mpibitonic {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Oddeven, ExecutionMethod.Mpi,
                        "/C mpiexec -n {0} mpioddeven {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Bucket, ExecutionMethod.Mpi,
                        "/C mpiexec -n {0} mpibucket {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Bitonic, ExecutionMethod.Cuda,
                        "/C cudabitonic.exe -g {1} -b {2} {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Oddeven, ExecutionMethod.Cuda,
                        "/C cudaoddeven.exe -g {1} -b {2} {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Bucket, ExecutionMethod.Cuda,
                        "/C cudabucket.exe -g {1} -b {2} {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Bitonic, ExecutionMethod.Net,
                        "/C netbitonic.exe -n {3} {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Oddeven, ExecutionMethod.Net,
                        "/C netoddeven.exe -n {3} {4} {5} >> sorting.log"
                    },
                    new object[]
                    {
                        SortingAlgorithm.Bucket, ExecutionMethod.Net,
                        "/C netbucket.exe -n {3} {4} {5} >> sorting.log"
                    }
                }
            ...
            var command = string.Format(commandFormat, numberOfProcess, gridSize, blockSize, degreeOfParallelism,
                inputFileName,
                outputFileName);
